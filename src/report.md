...
- **docker exec** - запустить команду в запущенном образе
    - **docker exec -it mydocker sh** как пример, запускает консоль (bash, sh) в интерактивном режиме (-it) на контейнере, что позволить прожимать команды прямо во включённом образе
- **
